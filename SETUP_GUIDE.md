...
│   │   ├── hooks/       # React hooks for API/WebSocket
│   │   ├── lib/         # API client and WebSocket manager
...
│   ├── services/        # External services (AQI, weather, SMS)
│   ├── simulation/      # Crisis simulation tools
...
```powershell
cd backend
python -c "from core.database import engine, Base; Base.metadata.create_all(bind=engine); print('✓ Database initialized')"
```
...
1. In the dashboard, change the scenario (Normal → Pollution → Festival → Outbreak)
